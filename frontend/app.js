...
function icon(name,size=18){
  const paths={
    ...
    logout:'<path d="M10 5H5a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h5"/><path d="M14 8l4 4-4 4"/><path d="M18 12H9"/>',
...
function themeToggle(){
  ...
  document.body.classList.remove('nova-theme-switching');

  void document.body.offsetWidth;

  document.body.classList.add('nova-theme-switching');
...
function showNotifications(){
  const existing=$('#notification-modal');

  if(existing){
    existing.classList.remove('nova-notification-opening');
    existing.classList.add('nova-notification-closing');
  ...
  root.id='notification-modal';
  root.className='modal-backdrop nova-notification-backdrop';
  ...
  root.innerHTML=`
    ...
    >
      <div class="modal-head">
        ...
        <div>
          <span class="eyebrow">CAMPUS NOVA</span>
          <h2>Уведомления</h2>
          <p class="modal-sub">
  ...
  novaFrame(()=>{
    root.classList.add('nova-notification-opening');
  });

  const close=()=>{
    root.classList.remove('nova-notification-opening');
    root.classList.add('nova-notification-closing');
      ...
      <aside class="sidebar">
        ...
        <div class="sidebar-bottom">
          ...
          <span class="connection">
            <i></i>
            ${state.demo?'Демо-режим':'Campus подключён'}
          </span>
      ...
      <main class="main">
        ...
        <header class="topbar">
          ...
          <div class="top-actions">
            ...
            <button
              class="icon-btn theme-top-button"
              id="theme-top"
              title="Сменить тему"
            ...
            <button
              class="icon-btn notification-button ${notificationsBadge()?'has-dot':''}"
              id="notifications"
              title="Уведомления"
              aria-label="Уведомления"
            >
    ...
    return `
      <section class="page profile-page">
        ${PageHead({
          eyebrow:'АККАУНТ',
          title:'Профиль',
          sub:'Загружаем данные из Campus…'
        })}
        ${skeletonGrid(2)}
      </section>
    `;
    ...
    return `
      <section class="page profile-page">
        ${PageHead({
          eyebrow:'АККАУНТ',
          title:'Профиль',
          sub:'Не удалось загрузить профиль.'
        })}
        ${statePanel('error','profile')}
      </section>
    `;
  }
  ...
  return `
    <section class="page profile-page">
      ...
      ${PageHead({
        eyebrow:'АККАУНТ',
        title:'Профиль',
        sub:'Твои данные и подключение Campus.',
        children:`
          <button
            class="nova-danger-button"
            id="profile-page-logout"
          >
            ${icon('logout',15)}
            Выйти
          </button>
      ...
      <div class="profile-v2">
        ...
        <section class="profile-card profile-identity-card">
          ...
          <div class="profile-identity-meta">
            ...
            <div>
              ...
              <b>${esc(p.id || state.user?.id || 'Не указан')}</b>
            </div>
        ...
        <section class="profile-card profile-overview-card">
          ...
          <div class="profile-stat-grid">
            ...
            <div class="profile-stat">
              <span>${icon('grid',15)} Курсы</span>
              <b>${courses.length}</b>
              <small>доступно в Nova</small>
            </div>

            <div class="profile-stat">
              <span>${icon('folder',15)} Campus</span>
              <b>Онлайн</b>
              <small>${esc(campusHost())}</small>
            </div>

            <div class="profile-stat">
              <span>${icon('user',15)} Роль</span>
              <b>Студент</b>
              <small>учебный профиль</small>
            </div>
        ...
        <section class="profile-card profile-courses-card">
          ...
          ${
            courseNames.length
                  ...
                  ${courseNames.map(
                    (name,index)=>`
                      <div class="profile-course-row">
                        <span class="profile-course-number">
                          ${String(index+1).padStart(2,'0')}
                        </span>

                        <span>
                          <b>${esc(name)}</b>
                          <small>Учебный курс Campus</small>
                        </span>

                        <span class="profile-course-arrow">
                          ${icon('arrow',14)}
                        </span>
                      </div>
                    `
              ...
              `
              : `
                <div class="profile-empty-courses">
                  ${icon('grid',20)}
                  <span>
                    Курсы ещё загружаются.
                  </span>
                </div>
              `
          }
        ...
        <section class="profile-card profile-details-card">
          ...
          <div class="profile-detail-list">
            ...
            <div>
              <span>Тема</span>
              <b>
                ${state.theme==='dark'?'Тёмная':'Светлая'}
              </b>
            </div>
          ...
          <button
            class="secondary wide profile-theme-button"
            id="theme-profile"
          >
            ${icon(
              state.theme==='dark'
...
function bind(){
  ...
  $('#theme-sidebar')?.addEventListener('click',themeToggle);
  $('#theme-top')?.addEventListener('click',themeToggle);
  $('#theme-profile')?.addEventListener('click',themeToggle);
  ...
  $('#calendar-today')?.addEventListener('click',()=>{
    ...
    render();
    loadData('calendar',true);
  });

  $('#expand-all')?.addEventListener(
    'click',
    ()=>$$('.course-section')
      .forEach(x=>x.open=true)
  );

  $('#collapse-all')?.addEventListener(
    'click',
    ()=>$$('.course-section')
      .forEach(x=>x.open=false)
  );

  $('#cards-mode')?.addEventListener(
    'click',
    ()=>{
      state.courseView='cards';
      localStorage.setItem(
        'nova-course-view',
        'cards'
      );
  ...
  $('#profile-menu-trigger')?.addEventListener(
    'click',
    toggleProfileMenu
  );

  $('#profile-dropdown-logout')?.addEventListener(
    'click',
    confirmLogout
  );

  $('#sidebar-logout')?.addEventListener(
    'click',
    confirmLogout
  );

  $('#profile-page-logout')?.addEventListener(
    'click',
    confirmLogout
  );

  $('#profile-bottom-logout')?.addEventListener(
    'click',
    confirmLogout
  );
  ...
  if(!window.__novaProfileMenuBound){
    window.__novaProfileMenuBound=true;
...
function toggleProfileMenu(event){
  ...
  const next=!wrap.classList.contains(
    'profile-menu-open'
  );
...
function confirmLogout(){
  ...
  if($('#nova-logout-confirm')){
    return;
  }

  const root=document.createElement('div');

  root.id='nova-logout-confirm';
  root.className='modal-backdrop nova-confirm-backdrop';

  root.innerHTML=`
    <div
      class="modal nova-confirm-modal"
      role="dialog"
      aria-modal="true"
      aria-labelledby="nova-logout-title"
    >

      <div class="nova-confirm-icon">
        ${icon('logout',21)}
      </div>
      ...
      <p>
        Подключение к Campus будет завершено.
  ...
  novaFrame(()=>{
    root.classList.add(
      'nova-confirm-opening'
    );
  });

  const close=()=>{
    root.classList.add(
      'nova-confirm-closing'
    );
  ...
  $('#nova-logout-cancel')?.addEventListener(
    'click',
    close
  );

  $('#nova-logout-confirm')?.addEventListener(
    'click',
    async()=>{
      const btn=$('#nova-logout-confirm');
...
async function loadRouteData(force=false,epoch=state.routeEpoch){
  ...
  if(r==='profile'){
    loadData('profile',force,epoch);
    if(!state.data.courses){
      loadData('courses',force,epoch);
    }
    return;
  }
